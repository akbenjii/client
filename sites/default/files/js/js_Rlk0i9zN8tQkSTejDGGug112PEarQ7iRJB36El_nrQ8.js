  ...
  $.flexslider = function(el, options) {
    ...
    methods = {
      init: function() {
        ...
        slider.container = $(slider.containerSelector, slider).first();
...
;
/**
 * JavaScript behaviors for the front-end display of membership landing page
 */
try {
...
(function($) {
    ...
    Drupal.behaviors.membership_landing = Drupal.behaviors.membership_landing || {};

    Drupal.behaviors.membership_landing.attach = function(context) {

        Drupal.membership_landing.init(context);
    };

    Drupal.membership_landing = Drupal.membership_landing || {};

    /* Custom Easing
    ------------------------------------------------------------------*/
    Drupal.membership_landing.easing = 'easeInOutQuint';

    Drupal.membership_landing.init = function(context) {
        var self = this;

        // Initialize Flexsliders
        $('.member-benefits-images ul').addClass('slides');

        if ($('.benefit-thumbnails .benefit-thumbnail').length > 0) {
            $('.benefit-thumbnails .benefit-thumbnail').click(function(e) {
                $('.benefit-thumbnails .benefit-thumbnail').removeClass('active');
                $(this).addClass('active');
                //show active content
                var index = $('.benefit-thumbnails .benefit-thumbnail').index(this);
                $('.subject-content .member-benefits > ul > li.views-row').removeClass('active');
                currBenefit = $('.subject-content .member-benefits > ul > li.views-row-' + (index + 1))
                currBenefit.addClass('active');

                Drupal.membership_landing.initBenefitImagesSlider(currBenefit);
            });

            $('.benefit-thumbnails .benefit-thumbnail:first-child').click();
        }
    }

    Drupal.membership_landing.initBenefitImagesSlider = function(slider) {
        slider.find('.member-benefits-images').flexslider({
            animation: 'fade',
            animationSpeed: 400,
            animationLoop: true,
            slideshowSpeed: 3000,
            ...
            directionNav: false,
            touch: false,
            easing: Drupal.membership_landing.easing
        });
    }
